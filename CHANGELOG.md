...
- **Иконка приложения** — заменена на пользовательскую (файл `icon.avif`), сконвертирована в PNG и применена через `flutter_launcher_icons` во всех плотностях экрана (mdpi → xxxhdpi) с адаптивной версией для Android 8+
- **Фото профиля** — добавлен реальный аватар (`assets/images/avatar.jpg`) вместо заглушки
- **Резюме** — добавлен PDF-файл резюме (`assets/documents/resume.pdf`), доступный для скачивания через кнопку на экране профиля

### Технические изменения

- Ассеты перенесены из `user-files/` в стандартную структуру Flutter: `assets/images/` и `assets/documents/`
- Удалён дублирующий файл резюме с кириллическим именем
